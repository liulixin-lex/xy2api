...
export XAI_API_KEY="${apiKey}"`
  ...
  // Shape follows Grok Build user guide (~/.grok/docs + custom-models) and production-ready XY2API setups.
  ...
  const configContent = `# Grok Build CLI → XY2API Grok group (API key auth).
# Docs: ~/.grok/docs/user-guide/05-configuration.md + 11-custom-models.md
# Verify after save: grok inspect
#
# IMPORTANT: api_backend must be "responses" for XY2API Grok (POST /v1/responses).
...
# Prefer API key when using a custom gateway (matches XY2API).
...
description = "Grok 4.5 via XY2API (Responses)"
...
# Imagine tools: model IDs go to XY2API media endpoints (not the text [model.*] catalog).
...
function generateGrokCodexFiles(baseUrl: string, apiKey: string): FileConfig[] {
  ...
  switch (shell) {
    case 'cmd':
      envPath = 'Command Prompt'
      envContent = `set XY2API_API_KEY=${apiKey}`
      break
    case 'powershell':
    case 'windows':
      envPath = 'PowerShell'
      envContent = `$env:XY2API_API_KEY="${apiKey}"`
      break
    default:
      envPath = 'Terminal'
      envContent = `export XY2API_API_KEY="${apiKey}"`
  }

  const configContent = `# Codex CLI → XY2API Grok group
...
# Grok/XY2API path is HTTP/SSE; disable WS (Codex may otherwise try WebSocket first)
...
function generateRoutedCodexFiles(
  ...
  const envContent = isWindows
    ? `$env:XY2API_API_KEY="${apiKey}"`
    : `export XY2API_API_KEY="${apiKey}"`

  const configContent = `# Codex CLI -> XY2API ${label} group
model_provider = "xy2api"
model = "${model}"
review_model = "${model}"
disable_response_storage = true
...
function generateOpenCodeConfig(platform: string, baseUrl: string, apiKey: string, pathLabel?: string): FileConfig {
  ...
  if (platform === 'gemini') {
    ...
    // Custom provider pointing at XY2API OpenAI-compatible Responses/Chat endpoints.
    provider[platform].npm = '@ai-sdk/openai-compatible'
    provider[platform].name = 'Grok via XY2API'
