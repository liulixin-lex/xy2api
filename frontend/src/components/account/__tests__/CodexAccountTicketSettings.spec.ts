...
vi.mock('@/api/admin/codexTicketProxies', () => ({ getTicketProxyPool: vi.fn().mockResolvedValue({ revision: 'p1', entries: [] }) }))
...
describe('CodexAccountTicketSettings', () => {
  ...
  it('persists a toggle immediately without submitting parameter drafts', async () => {
    api.getCodexAccountTicket.mockResolvedValue(makeStatus({ proxy_configured: true }))
    api.saveCodexAccountTicket.mockResolvedValue(makeStatus({ enabled: true, proxy_configured: true, state: 'waiting', config_revision: 'r2' }))
    const wrapper = mountCard()
    await flushPromises()
    await wrapper.get(selector('plan')).setValue('team')
    await wrapper.get(selector('enabled')).trigger('click')
    await flushPromises()
    expect(api.saveCodexAccountTicket).toHaveBeenCalledWith(4, { enabled: true, expected_revision: 'r1' })
  ...
  it('disables while harvesting even with invalid drafts and an unsaved business proxy', async () => {
    api.getCodexAccountTicket.mockResolvedValue(makeStatus({ enabled: true, state: 'harvesting' }))
    api.saveCodexAccountTicket.mockResolvedValue(makeStatus({ enabled: false, config_revision: 'r2' }))
    const wrapper = mountCard()
    await flushPromises()
    await wrapper.setProps({ proxyChanged: true })
    await wrapper.get('[data-testid="state-model-gpt-6-astra"]').setValue(false)
    await wrapper.get(selector('enabled')).trigger('click')
    await flushPromises()
    expect(api.saveCodexAccountTicket).toHaveBeenCalledWith(4, { enabled: false, expected_revision: 'r1' })
    expect(wrapper.get(selector('enabled')).attributes('aria-checked')).toBe('false')
    api.getCodexAccountTicket.mockResolvedValue(makeStatus({ config_revision: 'r2' }))
  ...
  it('resolves an ambiguous write by reading the authoritative state', async () => {
    api.saveCodexAccountTicket.mockRejectedValue(new Error('timeout'))
    const wrapper = mountCard()
    await flushPromises()
    api.getCodexAccountTicket.mockResolvedValue(makeStatus({ enabled: true, config_revision: 'r2' }))
    await wrapper.get(selector('enabled')).trigger('click')
    await flushPromises()
    expect(wrapper.get(selector('enabled')).attributes('aria-checked')).toBe('true')
  ...
  it('retries an explicit disable once on conflict but never retries enabling', async () => {
    api.getCodexAccountTicket.mockResolvedValue(makeStatus({ enabled: true }))
    const wrapper = mountCard()
    await flushPromises()
    api.getCodexAccountTicket.mockResolvedValue(makeStatus({ enabled: true, config_revision: 'r2' }))
    api.saveCodexAccountTicket.mockRejectedValueOnce({ status: 409 }).mockResolvedValueOnce(makeStatus({ config_revision: 'r3' }))
    await wrapper.get(selector('enabled')).trigger('click')
    await flushPromises()
    expect(api.saveCodexAccountTicket).toHaveBeenLastCalledWith(4, { enabled: false, expected_revision: 'r2' })
    api.saveCodexAccountTicket.mockClear().mockRejectedValue({ status: 409 })
    api.getCodexAccountTicket.mockResolvedValue(makeStatus({ config_revision: 'r4' }))
    await wrapper.get(selector('enabled')).trigger('click')
    await flushPromises()
    expect(api.saveCodexAccountTicket).toHaveBeenCalledTimes(1)
    expect(wrapper.get(selector('enabled')).attributes('aria-checked')).toBe('false')
    expect(wrapper.get('[role="alert"]').text()).toContain('saveFailed')
  })

  it('ignores a stale poll after a successful disable', async () => {
    api.getCodexAccountTicket.mockResolvedValue(makeStatus({ enabled: true }))
    const wrapper = mountCard()
    await flushPromises()
    let resolvePoll!: (value: CodexAccountTicketStatus) => void
    api.getCodexAccountTicket.mockReturnValueOnce(new Promise(resolve => { resolvePoll = resolve }))
    vi.advanceTimersByTime(3000)
    api.saveCodexAccountTicket.mockResolvedValue(makeStatus({ config_revision: 'r2' }))
    await wrapper.get(selector('enabled')).trigger('click')
    await flushPromises()
    resolvePoll(makeStatus({ enabled: true }))
    await flushPromises()
    expect(wrapper.get(selector('enabled')).attributes('aria-checked')).toBe('false')
  })
  ...
  it('requires saving a manual Team selection and sends it without changing the proxy', async () => {
    ...
    await flushPromises()
    expect(api.saveCodexAccountTicket).toHaveBeenCalledWith(4, { expected_revision: 'r1', ticket_plan: 'team', models: ['gpt-6-astra'], missing_policy: 'block' })
    expect((wrapper.get(selector('plan')).element as HTMLSelectElement).value).toBe('team')
    expect(wrapper.get(selector('save')).attributes('disabled')).toBeDefined()
  ...
  it('acquires nonblocking and blocks repeat or stale-configuration requests', async () => {
    ...
    await flushPromises()
    expect(api.harvestCodexAccountTicket).toHaveBeenCalledWith(4, undefined, undefined, expect.any(String))
  ...
  it('keeps model and policy drafts during polling and harvests one saved model', async () => {
    const base = makeStatus({ enabled: true, proxy_configured: true, models: ['gpt-6-astra'], missing_policy: 'block', state: 'ready' })
    api.getCodexAccountTicket.mockResolvedValue(base)
    const wrapper = mountCard()
    await flushPromises()
    await wrapper.get('[data-testid="state-model-gpt-5.6-sol"]').setValue(true)
    await wrapper.get('[data-testid="state-missing-policy"]').setValue('allow_unprotected')
    await vi.advanceTimersByTimeAsync(3000)
    await flushPromises()
    expect((wrapper.get('[data-testid="state-model-gpt-5.6-sol"]').element as HTMLInputElement).checked).toBe(true)
    expect((wrapper.get('[data-testid="state-missing-policy"]').element as HTMLSelectElement).value).toBe('allow_unprotected')
    const updated = { ...base, models: ['gpt-6-astra', 'gpt-5.6-sol'], missing_policy: 'allow_unprotected', tickets: [makeStatus({ model: 'gpt-5.6-sol', enabled: true, state: 'waiting', protection: 'unprotected' })] }
    api.saveCodexAccountTicket.mockResolvedValue(updated)
    await wrapper.get(selector('save')).trigger('click')
    await flushPromises()
    expect(api.saveCodexAccountTicket).toHaveBeenLastCalledWith(4, { expected_revision: 'r1', ticket_plan: 'pro', models: ['gpt-6-astra', 'gpt-5.6-sol'], missing_policy: 'allow_unprotected' })
    api.harvestCodexAccountTicket.mockResolvedValue(updated)
    await wrapper.get('[data-testid="state-model-statuses"] button').trigger('click')
    await flushPromises()
    expect(api.harvestCodexAccountTicket).toHaveBeenLastCalledWith(4, 'gpt-5.6-sol', undefined, expect.any(String))
    expect(wrapper.get('[data-testid="state-model-statuses"]').text()).toContain('protections.unprotected')
  })

  it('pauses polling in a hidden document and resumes without losing the draft', async () => {
    const wrapper = mountCard()
    await flushPromises()
    await wrapper.get(selector('plan')).setValue('team')
  ...
  it('separates replay from inconclusive business observation and retains stale data on refresh failure', async () => {
    const ticket = makeStatus({ enabled: true, ticket_usable: true, model_verified: true, last_business_result: 'oversized', last_business_at: '2026-09-19T12:00:00Z', updated_at: '2026-09-19T12:01:00Z', observed_length: 292, counters: { business_checked: 2, business_unconfirmed: 1 } })
    api.getCodexAccountTicket.mockResolvedValue({ ...ticket, tickets: [ticket] })
    const wrapper = mountCard()
    await flushPromises()
    expect(wrapper.get('[data-testid="state-business-result"]').text()).toContain('stateTicket.unconfirmed')
    expect(wrapper.get('[data-testid="state-diagnostics"]').text()).toContain('businessResults.oversized')
    expect(wrapper.get('[data-testid="state-model-statuses"]').text()).toContain('captureVerification')
    api.getCodexAccountTicket.mockRejectedValue(new Error('private'))
    await vi.advanceTimersByTimeAsync(3000)
    await flushPromises()
    expect(wrapper.get('[role="alert"]').text()).toContain('loadFailed')
    expect(wrapper.get('[data-testid="state-last-updated"]').text()).toContain('updatedAt')
    expect(wrapper.get('[data-testid="state-business-result"]').text()).toContain('stateTicket.unconfirmed')
    expect(wrapper.get('[data-testid="state-diagnostics"]').text()).toContain('businessResults.oversized')
