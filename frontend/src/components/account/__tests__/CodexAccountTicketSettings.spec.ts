...
describe('CodexAccountTicketSettings', () => {
  ...
  it('opts in an account using the global pool without account proxy controls', async () => {
    ...
    await flushPromises()
    expect(api.saveCodexAccountTicket).toHaveBeenCalledWith(4, { enabled: true, ticket_plan: 'pro', models: ['gpt-6-astra'], missing_policy: 'block' })
  ...
  it('requires saving a manual Team selection and sends it without changing the proxy', async () => {
    ...
    await wrapper.get(selector('save')).trigger('click')
    await flushPromises()
    expect(api.saveCodexAccountTicket).toHaveBeenCalledWith(4, { enabled: true, ticket_plan: 'team', models: ['gpt-6-astra'], missing_policy: 'block' })
    expect((wrapper.get(selector('plan')).element as HTMLSelectElement).value).toBe('team')
    expect(wrapper.get(selector('save')).attributes('disabled')).toBeDefined()
  ...
  it('acquires nonblocking and blocks repeat or stale-configuration requests', async () => {
    ...
    await flushPromises()
    expect(api.harvestCodexAccountTicket).toHaveBeenCalledWith(4, undefined)
    expect(wrapper.get(selector('harvest')).attributes('disabled')).toBeDefined()
  ...
  it('keeps model and policy drafts during polling and harvests one saved model', async () => {
    ...
    await wrapper.get(selector('save')).trigger('click')
    await flushPromises()
    expect(api.saveCodexAccountTicket).toHaveBeenLastCalledWith(4, { enabled: true, ticket_plan: 'pro', models: ['gpt-6-astra', 'gpt-5.6-sol'], missing_policy: 'allow_unprotected' })
    ...
    await flushPromises()
    expect(api.harvestCodexAccountTicket).toHaveBeenLastCalledWith(4, 'gpt-5.6-sol')
