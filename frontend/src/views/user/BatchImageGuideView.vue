...
// Persistent compatibility name: existing IndexedDB thumbnails must remain readable.
  ...
  if (!key) return
	  ...
	  try {
        ...
        items: parsedItems.value,
	      ...
	      `xy2api-ui-${Date.now()}-${Math.random().toString(36).slice(2, 10)}`,
	    )
...
async function retryFailedJob(job: BatchImageJobRow | BatchImageJob) {
  ...
  try {
    ...
    const retryJob = await submitBatchImageJob(
      ...
      `xy2api-ui-retry-${job.id}-${Date.now()}-${Math.random().toString(36).slice(2, 10)}`,
    )
