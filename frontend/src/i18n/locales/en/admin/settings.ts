export default {
    ...
    stateProxies: {
      ...
      linuxdo: {
        ...
        description: 'Configure LinuxDo Connect OAuth for XY2API end-user login',
      ...
      dingtalk: {
        ...
        description: 'Configure DingTalk OAuth for XY2API end-user login',
      ...
      scheduling: {
        ...
        accountSchedulingThresholdsDescription: 'When an account\'s current native usage window (OpenAI Codex/Anthropic session, or Grok request/token utilization) reaches this percent, XY2API temporarily removes it from scheduling until the window resets. Use 100 to disable.',
      ...
      upstreamBillingProbe: {
        ...
        description: 'Periodically retrieve rates declared by upstream XY2API sites. Account rates change only when the separate sync switch is enabled.',
      ...
      payment: {
        ...
        providerStripe: 'Stripe Embedded',
        providerStripeHosted: 'Stripe Hosted',
        stripeMode: 'Stripe checkout mode',
        stripeModeOff: 'Off',
        stripeModeHint: 'Embedded: pay on this site. Hosted: redirect to Stripe Checkout. Save to apply to new orders. Customers see Stripe; existing orders keep their original flow.',
        ...
        easypayCustomMethodsHint: 'Add provider-specific methods supported by this EasyPay endpoint. The payment type is stored on XY2API orders; the upstream type is sent as EasyPay type.',
        ...
        stripeHostedSetup: 'Save and reopen to copy the complete Webhook URL generated by this site. Its final number identifies this provider configuration and may differ between deployments. Manage payment methods and branding in Stripe Dashboard.',
        stripeHostedRequirements: 'Before enabling, configure the HTTPS frontend URL and persist TOTP_ENCRYPTION_KEY. Use an API key and Webhook from the same Stripe account and test or live environment.',
        stripeHostedHistory: 'Once orders exist, credentials and currency are retained for payment confirmation and refunds, even after completion. To change them, create a new configuration and disable the old one.',
        stripeWebhookApiVersionHint: 'The SDK API version is {version}. Webhook versions in the same .dahlia release series are compatible; the dates need not match. Verify fulfillment with a test payment after setup.',
