export default {
  ...
  keys: {
    ...
    useKeyModal: {
      ...
      grok: {
        description:
          'Configure Grok CLI, Claude Code, Codex, or OpenCode to send requests through your XY2API Grok group. Text models use Responses; image/video use Imagine model IDs on media endpoints.',
        claudeDescription: 'Configure Claude Code to send Messages API traffic through your XY2API Grok group.',
        codexDescription: 'Configure Codex to send Responses API traffic through your XY2API Grok group.',
        configTomlHint:
        ...
        codexConfigTomlHint:
          'Official Codex: wire_api = "responses" only; prefer env_key over experimental_bearer_token; supports_websockets = false for non-OpenAI gateways (XY2API can still accept client WS and bridge to HTTP/SSE). Back up ~/.codex/config.toml before merge.',
        note:
        ...
        codexNote:
          'Export XY2API_API_KEY, save config.toml under ~/.codex (mkdir -p ~/.codex). Prefer env_key auth; do not commit secrets.',
        codexNoteWindows:
          'Set $env:XY2API_API_KEY, save config.toml under %USERPROFILE%\\.codex. Prefer env_key auth; do not commit secrets.',
      },
      deepseek: {
        ...
        codexNote: 'Export XY2API_API_KEY before starting Codex. The downloaded catalog contains model metadata only, not your API key.',
      },
      minimax: {
      ...
      composite: {
        ...
        codexConfigTomlHint: 'Download the model catalog below, save both files under the Codex config directory, and restart Codex.',
        codexNote: 'Export XY2API_API_KEY before starting Codex. Model requests are routed by the selected catalog slug.',
      },
      routedCodex: {
        ...
        note: 'Export XY2API_API_KEY before starting Codex. The downloaded catalog contains model metadata only, not your API key.',
      },
      codexModelCatalog: {
  ...
  usage: {
    ...
    cacheHitRateHint: 'Calculated from this request log: cached read tokens ÷ (input + cache creation + cached read tokens). Forced cache billing affects this ratio.',
    tokenSpeed: 'Speed',
    tokenSpeedHint: 'Estimated speed: output tokens ÷ (total duration − time to first token), in seconds. Output may include reasoning tokens; the first-token timing mode affects the estimate.',
    tokenSpeedTotalHint: 'No time to first token recorded; estimated using output tokens ÷ total seconds. Output may include reasoning tokens.',
  ...
  modelPlaza: {
    ...
    detail: {
      ...
      longContextPartialNote: 'Long-context tiers apply only to selected models; other models use the base tier. Official tiers are for reference only'
    },
    table: {
      model: 'Model',
      input: 'Input',
