export default {
  ...
  keys: {
    ...
    useKeyModal: {
      ...
      grok: {
        description:
          '配置 Grok CLI、Claude Code、Codex 或 OpenCode，让请求通过当前 XY2API Grok 分组发送。文本模型走 Responses；图片/视频使用 Imagine 模型 ID 与媒体端点。',
        claudeDescription: '配置 Claude Code，让 Messages API 请求通过当前 XY2API Grok 分组发送。',
        codexDescription: '配置 Codex，让 Responses API 请求通过当前 XY2API Grok 分组发送。',
        configTomlHint:
        ...
        codexConfigTomlHint:
          'Codex 官方：wire_api 仅支持 "responses"；优先 env_key，勿与 experimental_bearer_token 混用；非 OpenAI 网关默认 supports_websockets = false（XY2API 仍可接客户端 WS 并桥接到 HTTP/SSE）。合并前备份 ~/.codex/config.toml。',
        note:
        ...
        codexNote:
          '导出 XY2API_API_KEY，将 config.toml 保存到 ~/.codex（可用 mkdir -p ~/.codex）。优先 env_key，勿提交密钥。',
        codexNoteWindows:
          '设置 $env:XY2API_API_KEY，将 config.toml 保存到 %USERPROFILE%\\.codex。优先 env_key，勿提交密钥。'
      },
      deepseek: {
        ...
        codexNote: '启动 Codex 前先导出 XY2API_API_KEY。下载的目录只包含模型元数据，不包含 API Key。'
      },
      minimax: {
      ...
      composite: {
        ...
        codexConfigTomlHint: '下载下方模型目录，将两个文件保存到 Codex 配置目录后重启 Codex。',
        codexNote: '启动 Codex 前先导出 XY2API_API_KEY；分组会根据目录中选中的模型路由请求。'
      },
      routedCodex: {
        description: '使用当前路由分组的完整模型目录配置 Codex。',
        configTomlHint: '下载下方模型目录，将两个文件保存到 Codex 配置目录后重启 Codex。',
        note: '启动 Codex 前先导出 XY2API_API_KEY。下载的目录只包含模型元数据，不包含 API Key。'
      },
      codexModelCatalog: {
        title: 'Codex 模型目录',
  ...
  usage: {
    ...
    cacheHitRateHint: '按本次请求日志计算：缓存读取 Token ÷（输入 + 缓存创建 + 缓存读取 Token）。强制缓存计费会影响此比例。',
    tokenSpeed: '速度',
    tokenSpeedHint: '估算速度：输出 Token ÷（总耗时 − 首字耗时），耗时按秒换算。输出可能包含思考 Token，结果受首字统计口径影响。',
    tokenSpeedTotalHint: '缺少首字耗时，按总耗时估算：输出 Token ÷ 总秒数。输出可能包含思考 Token。',
    latencyDuration: '总耗时',
    time: '时间',
    ws: 'WS',
    stream: '流式',
