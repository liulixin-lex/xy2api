export default {
    ...
    stateProxies: {
      ...
      linuxdo: {
        ...
        description: '配置 LinuxDo Connect OAuth，用于 XY2API 用户登录',
      ...
      dingtalk: {
        title: '钉钉登录',
        description: '配置钉钉 OAuth，用于 XY2API 用户登录',
      ...
      scheduling: {
        ...
        accountSchedulingThresholdsDescription: '当账号当前原生用量窗口（OpenAI Codex/Anthropic 会话，或 Grok 请求/Token 利用率）达到该百分比时，XY2API 会临时将其移出调度，直到窗口重置。填 100 表示禁用。',
      ...
      upstreamBillingProbe: {
        title: '上游倍率自动探测',
        description: '定期获取 API Key 账号所连接上游 XY2API 站点声明的计费倍率；只有另行开启“同步上游声明倍率”的账号才会更新账号倍率。',
        enabled: '启用全局自动探测',
      ...
      payment: {
        ...
        providerWxpay: '微信官方',
        providerStripe: 'Stripe 站内',
        providerStripeHosted: 'Stripe 托管',
        stripeMode: 'Stripe 收款模式',
        stripeModeOff: '关闭',
        stripeModeHint: '站内：在本站完成付款。托管：跳转到 Stripe 收银台。保存后用于新订单，用户统一看到 Stripe；已有订单继续按原方式处理。',
        ...
        easypayCustomMethodsHint: '添加当前易支付服务商额外支持的支付方式。支付方式会记录到 XY2API 订单中，上游 type 会作为易支付 type 参数提交。',
        ...
        stripeHostedSetup: '首次保存后重新打开，复制本站生成的完整 Webhook 地址。地址末尾是本站服务商编号，不同部署可能不同。付款方式与品牌外观在 Stripe 后台配置。',
        stripeHostedRequirements: '启用前请设置站点的 HTTPS 前端地址，并持久化 TOTP_ENCRYPTION_KEY。密钥与 Webhook 必须属于同一 Stripe 账户和测试／正式环境。',
        stripeHostedHistory: '此配置产生订单后，密钥和币种将被保留用于到账确认及退款，即使订单已完成也不能直接修改。需要更换时，请新建配置并停用旧配置。',
        stripeWebhookApiVersionHint: 'SDK API 版本为 {version}。Webhook 可选择同属 .dahlia 系列的版本，日期不必完全相同；配置后请验证一次回调到账。',
