export default {
    accounts: {
      ...
      schedulable: '参与调度',
  iqCheck: '智商检测',
      iqEfforts: {"upstream_default":"上游默认","none":"不启用","minimal":"最低","low":"低","medium":"中","high":"高","xhigh":"更高","max":"最高"},
      iqHistoryFailed: "检测记录加载失败，请重试。",
      iqDownloadFailed: "诊断下载失败，请重试。",
      iqLastValid: '最近有效判定',
      iqRetainedAssessment: '本轮未取得有效答案，继续保留上次有效判定。',
      iqBudgetWarning: '当前日预算不足以覆盖全天周期及每轮一次补试，预算耗尽后会等待次日。',
  ...
  allIQStatuses: '全部智商',
      ...
      iqReportedModel: "上游报告模型",
      iqGrader: "判题版本",
      iqSchedule: "检测模式",
      iqFixed: "固定间隔",
      iqAdaptive: "节约模式",
      iqMaxInterval: "最大间隔（分钟）",
      iqDailyLimit: "每日检测上限（UTC）",
      iqTimeout: "检测超时（秒）",
      iqQuotaGroup: "共享配额组（可选）",
      iqQuotaGroupHint: "仅填写管理员配置的真实共享配额组；未配置的组会延期。",
      iqScheduleHint: "基础间隔理论每天 {count} 轮；节约模式最长排期 {interval} 分钟，另加排队和请求耗时。聪明／降智仅表示本题通过／未通过。",
      iqExecution: "执行",
...
"group_paused": "配额组已暂停，请处理额度后手动排队","minimum_interval": "最小间隔未到", "upstream_cooldown": "上游冷却", "daily_budget": "今日预算已用完", "group_daily_budget": "配额组今日预算已用完", "group_cooldown": "配额组冷却", "quota_group_unconfigured": "配额组未配置", "account_disabled": "账号人工停用", "account_expired": "账号过期", "account_cooldown": "账号健康冷却", "account_quota": "账号额度不足", "account_unavailable": "账号暂不可读", "concurrency_unavailable": "并发服务暂不可用", "account_busy": "账号繁忙", "start_unavailable": "任务暂未启动", "quota_exhausted": "额度不足，请处理后重新排队", "authentication_unavailable": "认证失败，请检查凭据", "permission_denied": "权限不足，请检查授权", "policy_denied": "上游拒绝，请联系服务方", "unsupported_model": "模型不支持，请修改配置", "unsupported_parameter": "参数不支持，请修改配置"},
      iqReasons: {
        ...
        unsupported_media_type: '上游返回了不支持的内容类型',
      ...
      iqDiagnosticLabels: {
        ...
        ignored_items: '忽略项目数',
        answer_source: '答案来源',
        first_byte_ms: '首字节耗时（毫秒）',
        total_ms: '总耗时（毫秒）',
 error_code: "上游错误码", error_type: "错误类型", retry_after: "上游允许重试时间", retry_after_unbounded: "重试等待超出范围，已暂停", input_tokens: "输入 Token", output_tokens: "输出 Token", reasoning_tokens: "推理 Token", retry_visibility: "传输重试可见性",  parser_version: '解析器版本', stage: '阶段', code: '原因代码', http_status: 'HTTP 状态', media_type: '内容类型', content_encoding: '压缩格式', protocol: '协议', transport: '传输方式', format_detected: '已兼容识别格式', event_type: '事件类型', event_index: '事件序号', field: '字段位置', offset: '字节位置', bytes_read: '读取字节数', request_id: '上游请求编号' },
      iqReason: '判定原因',
