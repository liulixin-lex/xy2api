...
func ticketTestAccount(id int64) *Account {
	proxyID := int64(7)
	return &Account{ID: id, Platform: PlatformOpenAI, Type: AccountTypeOAuth, Status: StatusActive, Schedulable: true, Concurrency: 2, ProxyID: &proxyID, Proxy: &Proxy{ID: 7, Protocol: "http", Host: "fixed.example.com", Port: 8080}, Credentials: map[string]any{"access_token": "tok", "chatgpt_account_id": "acc-1"}, Extra: map[string]any{codexAccountTicketConfigKey: codexAccountTicketConfig{Enabled: true, Model: openAICodexTicketDefaultModel, ProxyURL: "socks5h://user-sid-{sid}-t-5:secret@us.1024proxy.io:3000", Revision: "revision-1"}}}
}
...
func (r *codexTicketRefreshRepo) MutateCodexTicket(_ context.Context, id int64, limit int, fn CodexTicketMutation) (*Account, error) {
	r.mu.Lock()
	defer r.mu.Unlock()
	now := time.Now()
	active := 0
	for i := range r.accounts {
		for _, rt := range codexTicketRuntimes(&r.accounts[i]) {
	...
	for i := range r.accounts {
		if r.accounts[i].ID == id {
			...
			changed, e := fn(&a, active, now)
			if e != nil {
				return nil, e
			}
			if changed {
				r.accounts[i] = a
				r.updates = maps.Clone(a.Extra)
			}
			return &a, nil
		}
	}
