...
func TestCodexAccountTicketSaveFailurePreservesDurableTicket(t *testing.T) {
	...
	require.True(t, status.TicketUsable)
	require.Equal(t, "ready", status.State)
	require.Equal(t, "Could not publish verified STATE", status.LastError)
	require.True(t, status.ExpiresAt.Equal(old.ExpiresAt))
	live, err := repo.GetByID(context.Background(), 41)
	require.NoError(t, err)
...
func (r *codexTicketFailSaveRepo) MutateCodexTicket(ctx context.Context, id int64, limit int, fn CodexTicketMutation) (*Account, error) {
	return r.codexTicketRefreshRepo.MutateCodexTicket(ctx, id, limit, func(a *Account, active int, now time.Time) (bool, error) {
		before := parseOpenAICodexTicketFromAny(id, openAICodexTicketDefaultModel, a.Extra[openAICodexTicketExtraKey(openAICodexTicketDefaultModel)])
		changed, err := fn(a, active, now)
		if err != nil {
			return false, err
		}
		after := parseOpenAICodexTicketFromAny(id, openAICodexTicketDefaultModel, a.Extra[openAICodexTicketExtraKey(openAICodexTicketDefaultModel)])
		if after != nil && (before == nil || !after.CapturedAt.Equal(before.CapturedAt)) {
			return false, errors.New("simulated publication failure")
		}
		return changed, nil
	})
}
