...
func TestCodexTicketWatchdogResponseRecoversWithoutReplayingBusiness(t *testing.T) {
	for _, reason := range []string{"model_mismatch", "state_312"} {
		t.Run(reason, func(t *testing.T) {
			...
			if reason == "state_312" {
				payload = watchdogCompletedEvent("gpt-6-astra")
			}
...
func TestCodexTicketWatchdogCloseRejectsMalformedTrailer(t *testing.T) {
	...
	require.NoError(t, err)
	require.Empty(t, string(rest), "bounded close drain observed the malformed trailer")
}
...
func TestCodexTicketWatchdogPreservesFailedHarvestCooldown(t *testing.T) {
	...
	_, err := s.mutateCodexTicket(context.Background(), 41, 0, func(a *Account, _ int, _ time.Time) (bool, error) {
		saveCodexTicketRuntime(a, old.Model, codexTicketRuntime{RetryAfter: &job.retryAfter, LastError: job.lastError})
		return true, nil
	})
	require.NoError(t, err)
	s.invalidateCodexTicketFromResponse(receiptForCodexTicket(old), "model_mismatch")
