...
const (
	...
	githubRepo     = "liulixin-lex/xy2api"
...
func (s *UpdateService) applyReleaseAssets(ctx context.Context, releaseAssets []Asset) error {
	...
	tempDir, err := os.MkdirTemp(exeDir, ".xy2api-update-*")
	if err != nil {
	...
	newBinaryPath := filepath.Join(tempDir, "xy2api")
...
func (s *UpdateService) extractBinary(archivePath, destPath string) error {
	...
	if strings.Contains(archivePath, ".tar") {
		...
		for {
			...
			if baseName == "xy2api" || baseName == "xy2api.exe" || baseName == "sub2api" || baseName == "sub2api.exe" {
