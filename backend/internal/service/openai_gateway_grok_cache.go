...
const (
	...
	grokClientToolCacheOptInHeader   = "X-XY2API-Grok-Client-Tool-Cache"
	legacyGrokClientToolCacheHeader  = "X-Sub2API-Grok-Client-Tool-Cache"
...
func resolveGrokCacheIdentity(c *gin.Context, body []byte, explicitKey, upstreamModel string) string {
	...
	// upstream session identifiers derived by XY2API.
...
// the cache-capable route so an intermediate XY2API does not need to preserve
...
// The XY2API header (and its legacy Sub2API alias) is consumed locally because
// buildGrokResponsesRequest only
...
func applyGrokFreeRequestToolCacheRoute(c *gin.Context, body, intentSourceBody []byte, account *Account, cacheIdentity string) ([]byte, error) {
	...
	requestOptOut := false
	if c != nil {
		headerValue := strings.TrimSpace(c.GetHeader(grokClientToolCacheOptInHeader))
		if headerValue == "" {
			headerValue = strings.TrimSpace(c.GetHeader(legacyGrokClientToolCacheHeader))
		}
