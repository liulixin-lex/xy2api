...
func TestProxyUpdateInvalidatesBoundProbeSnapshotsAndEnqueuesOutboxAtomically(t *testing.T) {
	...
	mock.ExpectQuery(regexp.QuoteMeta("SELECT id FROM accounts WHERE proxy_id=$1 AND platform='openai' AND type<>'apikey' AND deleted_at IS NULL")).WithArgs(int64(9)).WillReturnRows(sqlmock.NewRows([]string{"id"}).AddRow(int64(19)))
