...
type Group struct {
	...
	// ShowExclusiveBadge holds the value of the "show_exclusive_badge" field.
	ShowExclusiveBadge bool `json:"show_exclusive_badge,omitempty"`
	// SystemPromptConfig holds the value of the "system_prompt_config" field.
	SystemPromptConfig domain.GroupSystemPromptConfig `json:"system_prompt_config,omitempty"`
	...
	// 长上下文阶梯计费范围：all=全部模型，selected=仅指定模型
	LongContextPricingScope string `json:"long_context_pricing_scope,omitempty"`
	// selected 范围下启用长上下文阶梯计费的模型名或尾部通配规则
...
func (*Group) scanValues(columns []string) ([]any, error) {
	...
	for i := range columns {
		...
		case group.FieldName, group.FieldDescription, group.FieldPeakStart, group.FieldPeakEnd, group.FieldStatus, group.FieldDuplicateOperationID, group.FieldPlatform, group.FieldSubscriptionType, group.FieldLongContextPricingScope, group.FieldDefaultMappedModel, group.FieldMaxReasoningEffort, group.FieldMaxReasoningEffortOverLimit:
...
func (_m *Group) assignValues(columns []string, values []any) error {
	...
	for i := range columns {
		...
		case group.FieldShowExclusiveBadge:
			if value, ok := values[i].(*sql.NullBool); !ok {
				return fmt.Errorf("unexpected type %T for field show_exclusive_badge", values[i])
			} else if value.Valid {
				_m.ShowExclusiveBadge = value.Bool
			}
		case group.FieldSystemPromptConfig:
			if value, ok := values[i].(*[]byte); !ok {
				return fmt.Errorf("unexpected type %T for field system_prompt_config", values[i])
			} else if value != nil && len(*value) > 0 {
				if err := json.Unmarshal(*value, &_m.SystemPromptConfig); err != nil {
					return fmt.Errorf("unmarshal field system_prompt_config: %w", err)
				}
			}
		...
		case group.FieldLongContextPricingScope:
			if value, ok := values[i].(*sql.NullString); !ok {
				return fmt.Errorf("unexpected type %T for field long_context_pricing_scope", values[i])
			} else if value.Valid {
				_m.LongContextPricingScope = value.String
			}
		case group.FieldLongContextPricingModels:
			if value, ok := values[i].(*[]byte); !ok {
				return fmt.Errorf("unexpected type %T for field long_context_pricing_models", values[i])
			} else if value != nil && len(*value) > 0 {
				if err := json.Unmarshal(*value, &_m.LongContextPricingModels); err != nil {
					return fmt.Errorf("unmarshal field long_context_pricing_models: %w", err)
				}
			}
...
func (_m *Group) String() string {
	...
	builder.WriteString(", ")
	builder.WriteString("show_exclusive_badge=")
	builder.WriteString(fmt.Sprintf("%v", _m.ShowExclusiveBadge))
	builder.WriteString(", ")
	builder.WriteString("system_prompt_config=")
	builder.WriteString(fmt.Sprintf("%v", _m.SystemPromptConfig))
	builder.WriteString(", ")
	...
	builder.WriteString(", ")
	builder.WriteString("long_context_pricing_scope=")
	builder.WriteString(_m.LongContextPricingScope)
	builder.WriteString(", ")
	builder.WriteString("long_context_pricing_models=")
	builder.WriteString(fmt.Sprintf("%v", _m.LongContextPricingModels))
	builder.WriteString(", ")
