...
### STATE 可靠性 0.1.5 发布执行（2026-09-21）

- 用户已明确授权推送、受保护 PR 合并和发版，覆盖上一轮仅本地交付的限制。来源仍为 `reliability-work` / `feat/state-reliability`，功能 PR #52；目标产品版本 0.1.5，兼容 Sub2API 0.2.6。远端基线为 1a4fd39c，未同步新的上游版本。
- 发布前并行审查发现代理池可能使用进程临时密钥持久化凭据；已暂缓合并，补固定共享加密密钥守卫及重启/多副本回归。旧代理迁移失败须保留原记录，不能用临时密钥写入后造成永久不可解密。
- 原四角色已备份到 `release-0.1.5/previous/`；后续 PR 检查、发布制品、隔离升级和回滚结果继续追加原验证账本。真实账号成功率灰度与生产部署不在本轮执行范围。
