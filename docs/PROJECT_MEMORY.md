...
- 发布候选位于 `fix/iq-simplification`，以 `63f0a036` / v0.0.12 为基线。产品 `VERSION` 和 `UPSTREAM_BASE.json.xy2api_version` 晋级为 0.0.13，Sub2API 兼容版本保持 0.2.4。
- 迁移 246 删除共享配额表。升级前需停止旧检测工作者并备份数据库；回退旧程序须恢复升级前数据库，不能只替换二进制。最终 PR、合并提交、标签、Release、五平台包和双架构镜像以 `/xy/artifacts/openai-iq-check/v013-release-result.json` 与原 `VERIFICATION.txt` 的实际结果为准。
...
| --- | --- |
| 仓库路径 | `/xy/xy2api` |
| 当前分支 | 原工作区 main/b07824af9；本次独立副本 fix/iq-reliability-v2 |
| 发布提交 | `v0.0.10` / `e695c0356c972f398045f87b28a9cccda8a66176`，PR #29 |
| 工作树 | 原工作区干净；reliability-work包含本轮未提交的稳定性实现、迁移、界面、测试和交接文档 |
| XY2API 产品版本 | 本轮起点`VERSION`与provenance均为`0.0.11`；稳定性改动尚未发布，不修改版本号 |
...
- `20260915-v0.0.13-release`：已获提交、推送和发版授权；正在将已验收的智商检测简化实现连同版本 0.0.13 通过受保护 PR 发布。生产部署不在范围内。
