...
- 三方预检发现 16 个冲突，其中 10 个路径未登记；当前先通过 `chore/upstream-v0.2.5-preflight` 扩充精确 manual-merge 策略和测试，再由标准同步脚本生成正式同步分支。
...
- `20260916-sub2api-v0.2.5-xy2api-v0.1.0`：正在执行预检策略 PR、标准三方同步、RC 验证和正式发布；固定事务目录为 `/xy/artifacts/upstream-sync-v0.2.5-xy2api-0.1.0/`。禁止生产部署和真实账号探测。
