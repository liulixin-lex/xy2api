...
### 用量导出与管理员指标 0.1.8 发布（2026-09-22）

- 用户授权合并 PR #58 后台导出与 #60 管理员指标并正式发版。#58 在 16/16 检查通过后合并为 `b5dd95473`；#60 合入该 main，手动合并管理员使用页导入与双方项目记忆，保留后台导出和两个管理员指标开关。
- 组合后的 88 项前端定向回归已通过；修复自动合并时用户页测试丢失 UsageTable 导入的问题，保留权限断言。完整远端 CI、生产构建、发布与隔离烟测结果记录在固定账本。
- 本轮版本 0.1.8，兼容基线保持 0.2.6；追加导出迁移252，后台导出默认关闭。管理员两个显示开关默认开启。原 main 工作区及生产环境不操作。
- 固定四角色继续位于 `/xy/artifacts/admin-usage-metrics/`；发布验证补充存入 `release-0.1.8/`，最终提交、CI、标签、Release、平台制品与镜像验证结果以字面执行账本为准。
...
- `20260922-release-0.1.8`：用户授权合并 PR #58/#60 并发版；#58 已合并，正在组合验证 #60 与版本 0.1.8，保持保护规则，不操作生产。
