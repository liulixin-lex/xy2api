...
| 数据库凭据 | user=`xy2api`, password=`xy2api`, dbname=`xy2api` |
...
psql -U xy2api -h 127.0.0.1 -d xy2api -f temp.sql
```
...
3. 无密码登录并重置
   ...
   ALTER USER xy2api WITH PASSWORD 'xy2api';
...
```bash
# 连接数据库
psql -U xy2api -h 127.0.0.1 -d xy2api
...
# 执行 SQL 文件
psql -U xy2api -h 127.0.0.1 -d xy2api -f migration.sql
```
...
- [项目仓库](https://github.com/liulixin-lex/xy2api)
