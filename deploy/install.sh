#!/bin/bash
#
# XY2API Installation Script
# XY2API 安装脚本
# Usage: curl -sSL https://raw.githubusercontent.com/liulixin-lex/xy2api/main/deploy/install.sh | bash
#
...
# Configuration
GITHUB_REPO="liulixin-lex/xy2api"
INSTALL_DIR="/opt/xy2api"
SERVICE_NAME="xy2api"
SERVICE_USER="xy2api"
CONFIG_DIR="/etc/xy2api"
...
declare -A MSG_ZH=(
    ...
    # Completion
    ["install_complete"]="XY2API 安装完成！"
    ["install_dir"]="安装目录"
    ["next_steps"]="后续步骤"
    ["step1_check_services"]="确保 PostgreSQL 和 Redis 正在运行："
    ["step2_start_service"]="启动 XY2API 服务："
    ...
    ["not_installed"]="XY2API 尚未安装，请先执行全新安装"
    ["fresh_install_hint"]="用法"

    # Uninstall
    ["uninstall_confirm"]="这将从系统中移除 XY2API。"
    ...
    ["uninstall_complete"]="XY2API 已卸载"

    # Help
    ["usage"]="用法"
    ["cmd_none"]="(无参数)"
    ["cmd_install"]="安装 XY2API"
    ["cmd_upgrade"]="升级到最新版本"
    ["cmd_uninstall"]="卸载 XY2API"
    ...
    ["server_config_desc"]="配置 XY2API 服务监听地址"
...
declare -A MSG_EN=(
    ...
    # Installation
    ["install_title"]="XY2API Installation Script"
    ...
    # Completion
    ["install_complete"]="XY2API installation completed!"
    ["install_dir"]="Installation directory"
    ["next_steps"]="NEXT STEPS"
    ["step1_check_services"]="Make sure PostgreSQL and Redis are running:"
    ["step2_start_service"]="Start XY2API service:"
    ...
    ["not_installed"]="XY2API is not installed. Please run a fresh install first"
    ["fresh_install_hint"]="Usage"

    # Uninstall
    ["uninstall_confirm"]="This will remove XY2API from your system."
    ...
    ["uninstall_complete"]="XY2API has been uninstalled"

    # Help
    ["usage"]="Usage"
    ["cmd_none"]="(none)"
    ["cmd_install"]="Install XY2API"
    ["cmd_upgrade"]="Upgrade to the latest version"
    ["cmd_uninstall"]="Remove XY2API"
    ...
    ["server_config_desc"]="Configure XY2API server listen address"
...
get_current_version() {
    if [ -f "$INSTALL_DIR/xy2api" ]; then
        # Use grep -E for better compatibility (works on macOS and Linux)
        "$INSTALL_DIR/xy2api" --version 2>/dev/null | grep -oE 'v?[0-9]+\.[0-9]+\.[0-9]+' | head -1 || echo "unknown"
    else
        echo "not_installed"
    fi
}
...
download_and_extract() {
    ...
    local archive_name="xy2api_${version_num}_${OS}_${ARCH}.tar.gz"
    ...
    # Copy binary
    cp "$TEMP_DIR/xy2api" "$INSTALL_DIR/xy2api"
    chmod +x "$INSTALL_DIR/xy2api"
    ...
    print_success "$(msg 'binary_installed') $INSTALL_DIR/xy2api"
}
...
install_service() {
    ...
    cat > /etc/systemd/system/xy2api.service << EOF
[Unit]
Description=XY2API - AI API Gateway Platform
Documentation=https://github.com/liulixin-lex/xy2api
...
User=xy2api
Group=xy2api
WorkingDirectory=/opt/xy2api
ExecStart=/opt/xy2api/xy2api
Restart=always
RestartSec=5
StandardOutput=journal
StandardError=journal
SyslogIdentifier=xy2api
...
start_service() {
    ...
    if systemctl start xy2api; then
        print_success "$(msg 'service_started')"
        return 0
    else
        print_error "$(msg 'service_start_failed')"
        print_info "sudo journalctl -u xy2api -n 50"
        return 1
    fi
}
...
enable_autostart() {
    ...
    if systemctl enable xy2api 2>/dev/null; then
...
print_completion() {
    ...
    echo "=============================================="
    echo ""
    echo "  $(msg 'cmd_status'):   sudo systemctl status xy2api"
    echo "  $(msg 'cmd_logs'):     sudo journalctl -u xy2api -f"
    echo "  $(msg 'cmd_restart'):  sudo systemctl restart xy2api"
    echo "  $(msg 'cmd_stop'):     sudo systemctl stop xy2api"
    echo ""
    echo "=============================================="
}
...
upgrade() {
    ...
    # Get current version
    CURRENT_VERSION=$("$INSTALL_DIR/xy2api" --version 2>/dev/null | grep -oE 'v?[0-9]+\.[0-9]+\.[0-9]+' || echo "unknown")
    print_info "$(msg 'current_version'): $CURRENT_VERSION"

    # Stop service
    if systemctl is-active --quiet xy2api; then
        print_info "$(msg 'stopping_service')"
        systemctl stop xy2api
    fi

    # Backup current binary
    cp "$INSTALL_DIR/xy2api" "$INSTALL_DIR/xy2api.backup"
    print_info "$(msg 'backup_created'): $INSTALL_DIR/xy2api.backup"
    ...
    # Set permissions
    chown "$SERVICE_USER:$SERVICE_USER" "$INSTALL_DIR/xy2api"

    # Start service
    print_info "$(msg 'starting_service')"
    systemctl start xy2api

    print_success "$(msg 'upgrade_complete')"
}

# Install specific version (for upgrade or rollback)
# Requires: XY2API must already be installed
install_version() {
    ...
    if [ -f "$INSTALL_DIR/xy2api" ]; then
        ...
        if [ "$current_version" != "unknown" ] && [ "$current_version" != "not_installed" ]; then
            backup_name="xy2api.backup.${current_version}"
        else
            backup_name="xy2api.backup.$(date +%Y%m%d%H%M%S)"
        fi
        cp "$INSTALL_DIR/xy2api" "$INSTALL_DIR/$backup_name"
...
uninstall() {
    ...
    print_info "$(msg 'stopping_service')"
    systemctl stop xy2api 2>/dev/null || true
    systemctl disable xy2api 2>/dev/null || true

    print_info "$(msg 'removing_files')"
    rm -f /etc/systemd/system/xy2api.service
    systemctl daemon-reload
