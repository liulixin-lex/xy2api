# XY2API Deployment Files

This directory contains files for deploying XY2API on Linux servers and Apple-silicon Macs.
...
| `xy2api.service` | Systemd service unit file |
| `xy2api-datamanagementd.service` | datamanagementd systemd service unit file |
...
Apple-silicon Macs running macOS 26 can run the complete XY2API, PostgreSQL, and Redis stack with Apple `container` 1.1.0 or newer:
...
curl -sSL https://raw.githubusercontent.com/liulixin-lex/xy2api/main/deploy/docker-deploy.sh -o docker-deploy.sh
...
3. If `ADMIN_PASSWORD` is not set, check logs for the generated password:
   ```bash
   docker compose logs xy2api | grep "admin password"
   ```
...
# Restart XY2API only
docker compose -f docker-compose.local.yml restart xy2api
...
| `ADMIN_EMAIL` | No | `admin@xy2api.local` | Admin email |
...
docker compose -f docker-compose.local.yml down
cd ..
tar czf xy2api-complete.tar.gz deployment/

# Transfer to new server
scp xy2api-complete.tar.gz user@new-server:/path/to/destination/
...
XY2API supports three methods to connect to Gemini:
...
1. Download the latest release from [GitHub Releases](https://github.com/liulixin-lex/xy2api/releases)
2. Extract and copy the binary to `/opt/xy2api/`
3. Copy `xy2api.service` to `/etc/systemd/system/`
4. Run:
   ```bash
...
1. Edit the service file:
   ```bash
   sudo nano /etc/systemd/system/xy2api.service
   ```
...
The main config file is at `/etc/xy2api/config.yaml` (created by Setup Wizard).
...
/opt/xy2api/
├── xy2api              # Main binary
├── xy2api.backup       # Backup (after upgrade)
└── data/                # Runtime data
...
# View detailed logs
docker compose -f docker-compose.local.yml logs --tail=100 xy2api
...
# View detailed logs
docker compose logs --tail=100 xy2api
...
# View recent logs
sudo journalctl -u xy2api -n 50

# Check config file
sudo cat /etc/xy2api/config.yaml
...
XY2API supports TLS fingerprint simulation to make requests appear as if they come from the official Claude CLI (Node.js client).

> **💡 Tip:** Visit **[tls.xy2api.org](https://tls.xy2api.org/)** to get TLS fingerprint information for different devices and browsers.
