...
ENV_FILE="${XY2API_ENV_FILE:-${SCRIPT_DIR}/.env}"

STACK_LABEL_KEY="org.xy2api.stack"
STACK_LABEL_VALUE="apple-container"
NETWORK_NAME="xy2api-apple"
APP_CONTAINER="xy2api-apple"
POSTGRES_CONTAINER="xy2api-apple-postgres"
REDIS_CONTAINER="xy2api-apple-redis"
APP_VOLUME="xy2api-apple-data"
POSTGRES_VOLUME="xy2api-apple-postgres-data"
REDIS_VOLUME="xy2api-apple-redis-data"
PLATFORM="linux/arm64"

TEMP_DIR=""
LOCK_DIR="${TMPDIR:-/tmp}/xy2api-apple-container.lock"
LOCK_ACQUIRED=false
...
Commands:
  ...
  up [--recreate]       Create and start the complete XY2API stack
...
Environment:
  XY2API_ENV_FILE      Path to the deployment env file (default: deploy/.env)
EOF
}
...
acquire_lock() {
    if ! mkdir "${LOCK_DIR}" 2>/dev/null; then
        ...
        if [[ "${owner_pid}" =~ ^[0-9]+$ ]] && ! kill -0 "${owner_pid}" 2>/dev/null; then
            ...
            die "Another XY2API Apple container operation is already running."
        fi
    fi
    printf '%s\n' "$$" >"${LOCK_DIR}/pid"
    LOCK_ACQUIRED=true
...
cmd_init() {
    ...
    info "Review the file, then run: XY2API_ENV_FILE='${ENV_FILE}' ${SCRIPT_DIR}/apple-container.sh up"
}
...
prepare_environment() {
    ...
    APP_IMAGE="$(read_env_value APPLE_CONTAINER_XY2API_IMAGE ghcr.io/liulixin-lex/xy2api:latest)"
    ...
    POSTGRES_USER="$(read_env_value POSTGRES_USER xy2api)"
    POSTGRES_PASSWORD="$(read_env_value POSTGRES_PASSWORD)"
    POSTGRES_DB="$(read_env_value POSTGRES_DB xy2api)"
    ...
    TEMP_DIR="$(mktemp -d "${TMPDIR:-/tmp}/xy2api-apple.XXXXXX")"
...
APPLE_CONTAINER_XY2API_IMAGE_ID=${APP_IMAGE_ID}
EOF
...
create_app_container() {
    info "Creating XY2API container..."
    container create \
        --name "${APP_CONTAINER}" \
        --label "${STACK_LABEL_KEY}=${STACK_LABEL_VALUE}" \
        --network "${NETWORK_NAME}" \
...
set -e
mkdir -p "$DATA_DIR"
chown -R xy2api:xy2api "$DATA_DIR"

runtime_dir=/app/storage/runtime
runtime_binary="$runtime_dir/xy2api"
image_marker="$runtime_dir/base-image-id"
installed_image_id=""
if [ -f "$image_marker" ]; then
    installed_image_id="$(cat "$image_marker")"
fi
if [ ! -x "$runtime_binary" ] || [ "$installed_image_id" != "$APPLE_CONTAINER_XY2API_IMAGE_ID" ]; then
    mkdir -p "$runtime_dir"
    cp /app/xy2api "${runtime_binary}.new"
    chown xy2api:xy2api "${runtime_binary}.new"
    chmod 0755 "${runtime_binary}.new"
    mv "${runtime_binary}.new" "$runtime_binary"
    printf "%s\n" "$APPLE_CONTAINER_XY2API_IMAGE_ID" >"${image_marker}.new"
    mv "${image_marker}.new" "$image_marker"
    rm -f "${runtime_binary}.backup"
fi
chown -R xy2api:xy2api "$runtime_dir"
...
while true; do
    ...
    set -e
    child_pid=""
    echo "XY2API exited with status ${status}; restarting in 1 second..." >&2
    sleep 1
done
' \
        >/dev/null
...
start_app() {
    ...
    if ! wait_for_probe "XY2API" 180 probe_app; then
        show_failure_logs "${APP_CONTAINER}"
        die "XY2API did not become ready."
    fi
    if ! wait_for_probe "XY2API host port" 15 probe_host_app; then
...
cmd_up() {
    ...
    info "XY2API is available at http://${ACCESS_HOST}:${HOST_PORT}"
}
...
cmd_down() {
    ...
    info "XY2API stack stopped; persistent volumes were preserved."
}
...
cmd_logs() {
    ...
    case "${service}" in
        app|xy2api) container_name="${APP_CONTAINER}" ;;
...
confirm_destroy() {
    ...
    if [[ "${include_volumes}" == true ]]; then
        printf 'Delete the XY2API stack and all persistent data? [y/N] '
    else
        printf 'Delete the XY2API containers and network, preserving volumes? [y/N] '
    fi
    read -r answer
    [[ "${answer}" == "y" || "${answer}" == "Y" ]]
}
...
cmd_destroy() {
    ...
    if [[ "${include_volumes}" == true ]]; then
        ...
        info "XY2API stack and persistent data deleted."
    else
        info "XY2API stack deleted; persistent volumes were preserved."
    fi
}
