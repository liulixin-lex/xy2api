...
# 或：
#   sudo ./install-datamanagementd.sh --source /path/to/xy2api/repo

BIN_PATH=""
SOURCE_PATH=""
INSTALL_DIR="/opt/xy2api"
DATA_DIR="/var/lib/xy2api/datamanagement"
SERVICE_FILE_NAME="xy2api-datamanagementd.service"
...
参数:
  ...
  --source  指定 xy2api 仓库路径（脚本会执行 go build）
  -h, --help 显示帮助

示例:
  sudo ./install-datamanagementd.sh --binary ./datamanagement/datamanagementd
  sudo ./install-datamanagementd.sh --source /opt/xy2api-src
EOF
}
...
if ! id xy2api >/dev/null 2>&1; then
  echo "[2/6] 创建系统用户 xy2api..."
  useradd --system --no-create-home --shell /usr/sbin/nologin xy2api
else
  echo "[2/6] 系统用户 xy2api 已存在，跳过创建"
fi
...
echo "[4/6] 准备数据目录..."
mkdir -p "$DATA_DIR"
chown -R xy2api:xy2api /var/lib/xy2api
chmod 0750 "$DATA_DIR"
...
systemctl enable --now xy2api-datamanagementd

echo "[6/6] 完成，当前状态："
systemctl --no-pager --full status xy2api-datamanagementd || true

cat <<'EOF'

下一步建议：
1. 查看日志：sudo journalctl -u xy2api-datamanagementd -f
2. 在 xy2api（容器部署时）挂载 socket:
